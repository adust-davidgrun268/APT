#!/bin/bash
# APT two-stage pretraining script supporting both DDP and DeepSpeed back-ends.
#
# Usage examples:
#
#   # DDP (torchrun), both stages
#   ./scripts/train.sh --backend ddp --gpus 0,1,2,3 \
#       --config pretrain --va-name va_exp --vla-name vla_exp
#
#   # DeepSpeed ZeRO-2, both stages
#   ./scripts/train.sh --backend deepspeed --gpus 0,1,2,3 \
#       --config pretrain --va-name va_exp --vla-name vla_exp
#
#   # DeepSpeed ZeRO-3 + LoRA + gradient checkpointing, Stage-1 only
#   ./scripts/train.sh --backend deepspeed --gpus 0,1,2,3 --stage 1 \
#       --config pretrain --va-name va_exp --vla-name vla_lora \
#       --ds-zero 3 --vlm-mode lora --gc --vlm-lr 5e-6
#
#   # Resume Stage-1
#   ./scripts/train.sh --backend ddp --gpus 0,1,2,3 --stage 1 \
#       --config pretrain --vla-conti vla_exp
...
VA_SAVE_NAME="${VA_SAVE_NAME:-va_pretrain}"
VLA_SAVE_NAME="${VLA_SAVE_NAME:-vla_pretrain}"
...
Shared options:
  ...
  --config CONFIG_NAME        Training config (see apt/configs.py) [default: $CONFIG_NAME]
  --va-name NAME              Stage-0 save name [default: $VA_SAVE_NAME]
  --vla-name NAME             Stage-1 save name [default: $VLA_SAVE_NAME]
...
# Refuse to silently run without saving — empty -s makes checkpoints vanish.
if [[ ( "$STAGE" == "0" || "$STAGE" == "both" ) && -z "$VA_SAVE_NAME"  && -z "$VA_CONTI_NAME"  ]]; then
    echo "Error: Stage 0 requires --va-name <name> (or --va-conti <name> to resume)"; exit 1
fi
if [[ ( "$STAGE" == "1" || "$STAGE" == "both" ) && -z "$VLA_SAVE_NAME" && -z "$VLA_CONTI_NAME" ]]; then
    echo "Error: Stage 1 requires --vla-name <name> (or --vla-conti <name> to resume)"; exit 1
fi
...
# ── Print config ──────────────────────────────────────────────────────────────
echo "=========================================="
echo "APT Pretraining Configuration:"
...
launch() {
    local stage_cmd="$1"
    local stage_id="$2"
    local extra="$3"  # per-stage extra args (e.g. --load_from_va, --pretrained_ckpt)
...
# ── Stage 0: VA prior pretraining ─────────────────────────────────────────────
if [[ "$STAGE" == "0" || "$STAGE" == "both" ]]; then
    echo "=========================================="
    echo "STAGE 0: Pretraining (VA prior)"
    echo "=========================================="
...
# ── Stage 1: VLA likelihood alignment ─────────────────────────────────────────
if [[ "$STAGE" == "1" || "$STAGE" == "both" ]]; then
    echo "=========================================="
    echo "STAGE 1: Pretraining (VLA likelihood)"
    echo "=========================================="
    if [[ -n "$VA_CONTI_NAME" ]]; then
        STAGE0_CKPT="$VA_CONTI_NAME"
    else
        STAGE0_CKPT="$VA_SAVE_NAME"
    fi
    ...
    if [[ -n "$VLA_CONTI_NAME" ]]; then
        ...
        echo "Adapting from Stage-0 checkpoint: $STAGE0_CKPT"
        STAGE1_CMD="-s $VLA_SAVE_NAME"
        STAGE1_EXTRA="--pretrained_ckpt $STAGE0_CKPT --load_from_va"
    fi
