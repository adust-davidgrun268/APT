...
# Per-worker thread pool for parallel JPEG decoding.
# cv2.imdecode releases the GIL, so multiple threads can decode concurrently.
# Using a module-level executor means it is created once per worker process.
_JPEG_POOL: ThreadPoolExecutor | None = None

def _get_jpeg_pool() -> ThreadPoolExecutor:
    global _JPEG_POOL
...
def gather_frames(
    ...
    outputs = {
        "rgb": rgbs,  # (T, H, W, 3)
        "K": cam.camera.K.astype(np.float32), # (3, 3)
        "pose": cam_poses,  # (T, 4, 4)
    }
...
def slice_encoded_frames(
    ...
    """
    MEMORY OPTIMIZATION: Aggressive cleanup of intermediate arrays.
    """
    sampled: Dict[str, np.ndarray] = {}

    # Read K matrix with minimal memory footprint
    K_data = camera_group["K"][:]  # (3, 3)
    if K_data.ndim == 3:
        sampled["K"] = K_data[0].copy()  # Copy to release original array
        del K_data
    else:
        sampled["K"] = K_data
    ...
    for k in camera_group.keys():
        ...
        if dset.dtype == np.object_:
            ...
            if isinstance(first_sample, (str, bytes)):
                ...
                # Read all raw JPEG bytes sequentially (H5 access is not thread-safe),
                # then decode all frames in parallel (cv2.imdecode releases the GIL).
                raw_list = [dset[i] for i in ind_clipped]

                def _decode(raw):
                    return rearrange(jpeg_decode(raw), "h w c -> c h w")

                if len(raw_list) > 1:
                    pool = _get_jpeg_pool()
                    frames = list(pool.map(_decode, raw_list))
                else:
                    frames = [_decode(raw_list[0])]

                sampled[k] = np.stack(frames, axis=0)
        else:
            # raw data format
...
def slice_dset(dset: h5py.Dataset, indices: np.ndarray):
    ...
    # Variable-length (object) datasets don't support fancy indexing in h5py.
    if dset.dtype == np.object_:
        return np.stack([dset[i] for i in indices], axis=0)
    # Fixed-size datasets: sort indices for sequential I/O (far fewer disk seeks),
    # read in one call, then restore the original order.
    sort_order = np.argsort(indices, kind="stable")
    sorted_idx = indices[sort_order].tolist()
    data = dset[sorted_idx]  # single batched H5 read
    restore = np.argsort(sort_order, kind="stable")
    return data[restore]
