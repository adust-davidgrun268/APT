...
class MySimpleMHA(nn.Module):
    ...
    @classmethod
    def _embed_qkv(
        cls, q: Tensor, k: Tensor, v: Tensor, x_pe: Tensor, c_pe: Tensor, pe_type: str
    ):
        # x_pe can be x_wcT
        # c_pe can be c_wcT
        x_pe_inv = None
        if pe_type == "rope":
            ...
            if x_pe is not None:
                x_cwT = x_pe_inv = x_pe.inverse()  # always float32
                assert x_cwT.dtype == torch.float32
                q = pe.PRoPE.embed_q(q, x_cwT.to(q.dtype))
            if c_pe is not None:
                k = pe.PRoPE.embed_kv(k, c_pe.to(k.dtype))
                v = pe.PRoPE.embed_kv(v, c_pe.to(v.dtype))
        elif pe_type is None:
            pass  # do nothing
        else:
            raise TypeError("Unknown pe_type: {}".format(pe_type))
        return q, k, v, x_pe_inv
    
    @classmethod
    def _embed_o(
        cls, o: Tensor, x_pe_inv: Tensor, pe_type: str
    ):
        if pe_type == "prope":
            if x_pe_inv is not None:
                o = pe.PRoPE.embed_out(o, x_pe_inv.to(o.dtype))
        return o
    
    def embed_qkv(
        self, q: Tensor, k: Tensor, v: Tensor, x_pe: Tensor, c_pe: Tensor
    ):
        if self.pe_type is None:
            return q, k, v, None
        
        if isinstance(self.pe_type, str):
            return self._embed_qkv(q, k, v, x_pe, c_pe, self.pe_type)
        
        assert len(self.pe_type) == 2
        halfD = q.shape[-1] // 2
        q0, k0, v0, x_pe_inv0 = self._embed_qkv(q[..., :halfD], k[..., :halfD], v[..., :halfD], x_pe[0], c_pe[0], self.pe_type[0])
        q1, k1, v1, x_pe_inv1 = self._embed_qkv(q[..., halfD:], k[..., halfD:], v[..., halfD:], x_pe[1], c_pe[1], self.pe_type[1])
        q = torch.cat([q0, q1], dim=-1)
        k = torch.cat([k0, k1], dim=-1)
        v = torch.cat([v0, v1], dim=-1)
        x_pe_inv = (x_pe_inv0, x_pe_inv1)
        return q, k, v, x_pe_inv
    ...
    def embed_o(
        ...
        if isinstance(self.pe_type, str):
            return self._embed_o(o, x_pe_inv, self.pe_type)
        
        assert len(self.pe_type) == 2
        halfD = o.shape[-1] // 2
        o0 = self._embed_o(o[..., :halfD], x_pe_inv[0], self.pe_type[0])
        o1 = self._embed_o(o[..., halfD:], x_pe_inv[1], self.pe_type[1])
    ...
    def forward_eager(
        ...
        out, attn_weight, attn_mask = self._attn_impl_eager(
            q, k, v, 
    ...
    def forward_sdpa(
        ...
        out, attn_weight, attn_mask = self._attn_impl_sdpa(
            q, k, v,
            training=self.training,
            attn_mask=attn_mask,
            drop_attn=self.dropout,
