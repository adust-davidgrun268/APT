...
class Service(TrajPlanner):
    @expose()
    def run_inference(self, obs_data):
        return super()._run_inference(obs_data).cpu()
    ...
    @expose()
    def get_action(self, sample_num: int = 1, draw_traj: bool = False, compress_traj_img: bool = False):
        return super().get_action(sample_num, draw_traj, compress_traj_img)
...
def run_service():
    ...
    parser.add_argument("--use_va", action="store_true", default=False)
